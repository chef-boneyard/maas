...
if node['maas']['create_superuser']
  execute 'Setup Superuser' do
    command "maas-region-admin createadmin --username=#{node['maas']['superuser_name']} --password=#{node['maas']['superuser_password']} --email #{node['maas']['superuser_email']} && touch /etc/maas/.superuser_created"
